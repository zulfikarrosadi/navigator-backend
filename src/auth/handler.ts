...
import { JWT_Payload } from '../schema';
...
async function create(
  ...
  const accessToken = jwt.sign(result.data.users, JWT_SECRET, { expiresIn: ACCESS_TOKEN_EXPIRATION_TIME });
  const refreshToken = jwt.sign(result.data.users, JWT_SECRET, { expiresIn: REFRESH_TOKEN_EXPIRATION_TIME });
...
async function refresh(req: Request, res: Response) {
  ...
  try {
    ...
    const newAccessToken = jwt.sign({ id: decoded.id, username: decoded.username }, JWT_SECRET, { expiresIn: ACCESS_TOKEN_EXPIRATION_TIME })
